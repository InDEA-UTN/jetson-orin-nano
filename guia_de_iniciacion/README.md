...
| 04 | [`04_instalacion_en_microsd.md`](04_instalacion_en_microsd.md) | Grabar la imagen con Etcher, primer arranque, configuración de Ubuntu, subida a 6.2.2 y verificación. Incluye el camino por SDK Manager para firmware viejo. | **Ejecutado en la placa** (camino A, Etcher: 6.2.1 → 6.2.2 por `apt`) — faltan pegar las salidas reales |
...
| 08 | [`08_primer_ejemplo_de_inferencia.md`](08_primer_ejemplo_de_inferencia.md) | Un primer ejemplo de punta a punta: clasificar o detectar sobre el video de la cámara, y entender qué hizo cada parte. | **Verificado en la placa** — clasificación sobre imagen fija. La inferencia en vivo sobre la cámara está resuelta en [`../tutoriales/01_hello_ai_world.md`](../tutoriales/01_hello_ai_world.md) |
...
- Cuando se escribió esta guía el SSD todavía no estaba en el laboratorio. Llegó después y se
  instaló el **06/08/2026** ([`05_instalacion_en_ssd_nvme.md`](05_instalacion_en_ssd_nvme.md)),
  pero el orden de lectura se mantiene: la microSD sigue siendo el camino corto para el que
  arranca de cero.
...
El destino final es el SSD: es donde corre el proyecto de visión + IA y es el único lugar donde se
puede poner swap. Hoy es ya el medio de arranque por defecto. La microSD quedó intacta, para
proyectos generales y como respaldo de arranque —se elige con **F11**—, que es un lujo que conviene
tener.
